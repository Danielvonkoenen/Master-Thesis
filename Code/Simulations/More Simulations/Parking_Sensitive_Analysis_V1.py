...
NORMAL_SPACES = 0                # Amount of normal parking Spaces

#PV Properties
PV_COUNT = 425                     # Amount of PVs with 1 killowatt-peak equals 4 to 6 moduls, which together occupy a area of 8 to 10 square meters.
PV_GENERATION = 0.8               # PV Generation per 1 hour  - approx. 7 hours sunlight 1 killowatt-peak approx. 1,000 kw per year / 365 = 5 /7 = 0.4; Sonne
PV_STORE = 100000                    # Max Energy a PV Generator can store kwh

#Charging Stations Properties
CS_COUNT_1 = 17                    # Amount of Charging Stations Level 1
CS_COUNT_2 = 18                    # Amount of Charging Stations Level 2
...
BATTERY_LEVEL = 0.5              # Min levels of battery level (in volt)
...
EV_ADOPTION = 100                  # Percentage of EV Adoption


"A car arrives at the parking loot."
def arrival(name, env, parking_loot, car_type, p_space, num_tickets, ChargingStation, export_data, export_data1, export_data2):
    """
    ...
    with parking_loot.request() as req:                                     
        ...
        if p_space == 'Level 1':                                            # Check charger Level preference
          ...
          if not ChargingStation.availableL2[p_space]:                      # Check availability of the second charger Level preference
            ...
            yield env.process(charge_processL2(name, env, p_space, num_tickets, ChargingStation, export_data, export_data1, export_data2))
            return                                                          # The charging process was triggerd
          ...
          if not ChargingStation.available[p_space]:                        # Check if at least a normal parking space is free
            ...
            tmp = int(round(battery_capacity * BATTERY_LEVEL))
            battery_level = random.randint(tmp,battery_capacity-1)
...
def charge_processL2(name, env, p_space, num_tickets, ChargingStation, export_data, export_data1, export_data2):
    ...
    while True:                                                           # PV produce every hour a certain amount of energy
      ...
      if env.now >= 32400 and env.now <= 57600:                           # from 9 am - 4 pm       
        if env.now >= 39600 and env.now <= 52200:                         # from 11 am - 2:30 pm
          pv_generator.put((test3 ) * test2)             # Produce more energy and store it
          test.put(test3  * test2)

        else:
          pv_generator.put((test3) * test2)            # Produce less energy and store it
          test.put(test3 * test2)


      print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA --------------- %d" % (pv_generator.level))
      yield env.timeout(3600)                                             # Update every hour 
      
"A Vehicle / EV is generated for the Simulation"
def car_generator(env, parking_loot, charge_station, export_data, export_data1, export_data2):
    ...
    for i in itertools.count():                                             # Generate cars depending on the time of Day
        ...
        env.process(arrival('Car %d' % i, env, parking_loot, car_type, p_space, num_tickets, charge_station, export_data, export_data1, export_data2)) # Arrival process is triggered 
...
while i < 101:      #101
 ...
 test = simpy.Container(env, PV_STORE, init=0)
 ...
 env.process(car_generator(env, parking_loot, ParkingFacility, export_data, export_data1, export_data2))
 ...
 print (' %d was gatherd from the pv' % (pv_generator.level))
 print (' %d was gatherd from the pv' % (test.level))
 ...
 #Export data
 my_df.to_csv('25homechargewithevandPV +%i.csv' % (i), index=False, header=True)
 my_df1.to_csv('25chargeenergywithPV +%i.csv' % (i), index=False, header=True)
 #my_df2.to_csv('35PVEnergyConsumption +%i.csv' % (i), index=False, header=True)
 i  += 1
